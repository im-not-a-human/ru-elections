...
// Bio rendered into a <dialog>, opened by a neutral "Подробнее" button.
...
(function() {
    ...
    return (
      '<dialog class="lg-bio-dialog" id="lg-bio-dialog-' + idx + '" aria-labelledby="lg-bio-title-' + idx + '">' +
        '<div class="lg-bio-dialog-inner">' +
          '<button class="lg-bio-dialog-close" type="button" aria-label="Закрыть" data-close>×</button>' +
  ...
  window.renderLeaderGrid = function(rootEl, leaders) {
    ...
    if (core.length > 0) {
      html +=
        '<div class="lg-tier">' +
          '<div class="lg-tier-header">Ядро партии</div>' +
    ...
    if (secondary.length > 0) {
      html +=
        '<div class="lg-tier">' +
          '<div class="lg-tier-header">Остальные публичные фигуры</div>' +
