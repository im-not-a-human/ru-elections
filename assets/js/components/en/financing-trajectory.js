// EN translation of assets/js/components/financing-trajectory.js
// Sync source: assets/js/components/financing-trajectory.js
...
(function() {
  ...
  window.renderFinancingTrajectory = function(rootEl, data) {
    ...
    if (typeof Chart === 'undefined') {
      rootEl.innerHTML = '<div class="dok-error" style="padding:40px;text-align:center"><p>Chart.js not loaded — please refresh the page.</p></div>';
      return;
    }
    ...
    rootEl.innerHTML =
      '<div style="position:relative;width:100%;height:280px">' +
        '<canvas id="' + canvasId + '" role="img" ' +
          'aria-label="' + escHtml('New People party income from the state budget: ' +
          data.years.map(function(y, i) { return y + ': ' + data.budgetPct[i] + '%'; }).join(', ')) + '">' +
          'Chart of budget-funding share by year.' +
        '</canvas>' +
      '</div>' +
    ...
    data.years.forEach(function(year, i) {
      ...
      tag.setAttribute('aria-label', year + ': ' + pct + '% budget share');
      tag.dataset.hotspotTitle = (hs && hs.title) || (year + '');
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: function(ctx) {
                var i = ctx.dataIndex;
                var lines = [ctx.parsed.y + '% budget'];
                if (data.totalsMln && data.totalsMln[i] != null) {
                  lines.push('Total: ' + data.totalsMln[i] + ' mn ₽');
                }
                return lines;
              },
            },
