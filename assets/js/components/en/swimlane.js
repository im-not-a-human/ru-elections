// EN translation of assets/js/components/swimlane.js
// Sync source: assets/js/components/swimlane.js
...
(function() {
  ...
  window.renderSwimlane = function(rootEl, data) {
    ...
    if (data.connection && connFromIdx >= 0 && connToIdx >= 0) {
      ...
      var label = c.label || (deltaDays + ' days');
        ...
        if (i === connFromIdx) {
          rowHtml += (
            '<div class="cv-connection-note" role="note">' +
              ...
              '<span class="cv-conn-meta">' + (deltaDays >= 0 ? deltaDays : Math.abs(deltaDays)) + ' days</span>' +
            '</div>'
          );
        }
        return rowHtml;
    ...
    rootEl.innerHTML =
      '<div class="cv-timeline" role="region" aria-label="' + escAttr('Crisis-event timeline') + '">' +
        headerHtml +
