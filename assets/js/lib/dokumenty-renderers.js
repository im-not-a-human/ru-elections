...
(function() {
  ...
  function loadLib(url) {
    ...
    libCache[url] = new Promise(function(resolve, reject) {
      ...
      s.onerror = function() { reject(new Error('Не удалось загрузить ' + url)); };
  ...
  function renderCsv(bodyEl, url) {
    return fetch(url).then(function(r) {
      ...
      if (rows.length === 0) {
        bodyEl.innerHTML = '<div class="dok-empty"><p>Пустой CSV-файл.</p></div>';
        return;
      }
      ...
      html += '</tbody></table>';
      if (truncated) {
        html += '<p style="padding:12px;color:var(--ink-muted)">Показаны первые ' + (displayRows - 1) + ' строк (файл больше — превью обрезано).</p>';
      }
      html += '</div>';
  ...
  function renderMd(bodyEl, url) {
    bodyEl.innerHTML = '<div class="dok-loading">Загружаю marked.js…</div>';
  ...
  function wrapPathLinks(rootEl) {
    ...
    codes.forEach(function(code) {
      ...
      a.title = 'Открыть «' + clean + '» во вкладке';
