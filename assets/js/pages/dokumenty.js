// Init for /dokumenty.html — file explorer with tabs.
...
(function() {
    ...
    STATE.openPaths.forEach(function(path) {
      ...
      html +=
        ...
        'title="' + escHtml(node.name) + '">' +
          ...
          '<span class="close-x" data-close="' + escHtml(path) + '" aria-label="Закрыть вкладку" role="button" tabindex="0">' +
            closeHtml +
          '</span>' +
        '</button>';
    });
  ...
  function renderActivePreview() {
    ...
    if (!node) { showError('Файл не найден в дереве: ' + STATE.activePath); return; }
    ...
    if (EL.previewBody) {
      ...
      EL.previewBody.innerHTML =
        '<div class="dok-empty">' +
          ...
          '<p>Откройте файл в дереве слева — он откроется во вкладке.</p>' +
        '</div>';
    }
  }
  ...
  function updateFsButtonIcon() {
    ...
    btn.title = inFs ? 'Выйти из полноэкранного режима' : 'Полноэкранный режим';
  ...
  window.initDokumenty = function initDokumenty() {
    ...
    Promise.all([
      ...
      // Header count with Russian pluralization
      if (EL.totalCount) {
        var n = countFiles(STATE.tree);
        var mod10 = n % 10, mod100 = n % 100;
        var word = (mod10 === 1 && mod100 !== 11) ? 'файл'
                 : (mod10 >= 2 && mod10 <= 4 && (mod100 < 12 || mod100 > 14)) ? 'файла'
                 : 'файлов';
        EL.totalCount.textContent = n + ' ' + word;
      }
      ...
      EL.tree.innerHTML = '<div class="dok-error"><p>Ошибка загрузки дерева: ' + escHtml(err.message) + '</p></div>';
    });
